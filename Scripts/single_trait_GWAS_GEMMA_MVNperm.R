...
results_folder = sprintf('%s/dataset_%02d_%s/rep_%02d/GEMMA_univariate',trait,dataset,ifelse(transform==1,'orig','INT'),rep)
...
for(experimento in colnames(Y)) {
  ...
  # estimate S using rrBLUP
  res = mixed.solve(y,X=Xt,K=Ki)
  S = res$Vu*Ki + res$Ve*diag(1,length(y))
  y = mvnpermute(y,Xt,S,1)
  res2 = mixed.solve(y,X=Xt,K=Ki);res2$Vu/(res2$Vu+res2$Ve)
...
fwrite(as.data.frame(MAF[i,]),file = sprintf('MVNperm_MAF_chr%02d.txt',chr),row.names=T,col.names=T,sep=',',na = 'NA')
fwrite(as.data.frame(beta_hat[i,]),file = sprintf('MVNperm_beta_hat_chr%02d.txt',chr),row.names=T,col.names=T,sep=',',na = 'NA')
fwrite(as.data.frame(SE[i,]),file = sprintf('MVNperm_SE_chr%02d.txt',chr),row.names=T,col.names=T,sep=',',na = 'NA')
fwrite(as.data.frame(p_wald[i,]),file = sprintf('MVNperm_p_wald_chr%02d.txt',chr),row.names=T,col.names=T,sep=',',na = 'NA')
