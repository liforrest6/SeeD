...
## this block of code is commented out for validation set with no withheld accessions
sampled_data = data %>% sample_frac(.2)
validationIDs = sampled_data$Unique.ID
Y[which(data$Unique.ID %in% validationIDs),] = NA
write.csv(as.data.frame(validationIDs), file = sprintf('%s/withheld-accessions-rep_%02d.csv',results_folder, rep))
...
runID = paste0(dir, sprintf('/Analyses/MegaLMM_output/MegaLMM_runs/%s-validation_%d',dataset,rep))
...
write.csv(U_hat,file = sprintf('%s/U_hat-rep_%02d.csv',results_folder, rep))


# ## testing not using quotes
# U_hat2 = get_posterior_mean(MegaLMM_state,U_R + U_F %*% Lambda)
# write.csv(U_hat2,file = sprintf('%s/U_F.csv',results_folder))
# U_F = get_posterior_mean(load_posterior_param(MegaLMM_state,"U_F"))
# U_R = get_posterior_mean(load_posterior_param(MegaLMM_state,"U_R"))
# write.csv(U_F,file = sprintf('%s/U_F.csv',results_folder))
# write.csv(U_R,file = sprintf('%s/U_R.csv',results_folder))
