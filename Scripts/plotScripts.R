...
##### create maps for elevation and locations #####################################


## API key located in apikey.R
...
######### manuscript plots for environmental map
## pdf version
...
############## create plot for map on just Mexico
...
##### simple histograms of climate variables ######################################
...
## manhattan plot with top 32 lead SNPs for multivariate envGWAS for manuscript figure, printed later
manhplot = manual_manhattan(gea_results, top_hits_clumped,title = '')

### qqplot for multivariate envGWAS - does not plot with ggsave (no saved object) so use print function
...
## creating of manhattan plot for envGWAS without using population structure correction 
## also printed out separately with prediction results
...
## load all results from univariate results
...
gemma_variables = c('tmin', 'tmax', 'trange', 'precipTot', 'rhMean', 'aridityMean', 'elevation')


## doing this for manual
makeGEMMAPlots = lapply(1:length(gemma_variables), function(i) {
  this_gemma = manual_manhattan(gemma_results[i][[1]],
                                bp_col = 'BP',
                                p_col = variables[i],
                                chr_col = 'Chr',
                                title = sprintf('%s GEA', gemma_variables[i]),
                                highlight_SNP_list = c(inv4mSNPs, hsftf9SNPs))
  ## plotting of png
  png(here(plot_dir, sprintf('GEMMA_univariate_%s_manhattan_manual.png', gemma_variables[i])), height = 480, width = 720)
  print(this_gemma)
  dev.off()
  this_gemma
})

## read actual GEMMA plots if creating pdf to combine pngs together
readGEMMAPlots <- lapply(gemma_variables, function(i) {
  rasterGrob(readPNG(file.path(here(plot_dir, sprintf('GEMMA_univariate_%s_manhattan.png', i))), native = FALSE),
             interpolate = FALSE)
})

## making with png because otherwise the dot plots in the manhattan crash everything 
...
## must run phenotypicAnalysis.R first to process SNP data with phenotype data

## plotting GEA enrichment for manuscript - 
## samples SNPs based on maf and returns effect sizes for phenotype for each trait
...
## calculate p-values for each trait to calculate enrichment
for(i in 1:4){
...
#                                     , 'ASI'))
## calculate pvals for each trait
...
# print(fig4)
# dev.off()
enrichment_palette = c('#264653', '#2a9d8f', '#8ab17d', '#e9c46a', '#f4a261', '#e76f51')
...
(enrichment_by_violin = ggplot(allTraits_pvals %>% 
  ...
  theme(legend.text = element_text(size = 15),
        ...
        plot.title = element_text(size = 20)) +
  scale_fill_manual(values = enrichment_palette, guide = 'none') +
...
## run gea-analysis.R first to obtain GEA SNPS
...
all_traits_phenotypic_byModel_aggregate = lapply(traits[-c(1, 2)],
                                                 create_prediction_comparison_plot_byModel,
                                                 blup_style = 'deregressed_blups_nostress_filterTrial',
                                                 columns = c('lm_PC5', 'lm_matching_SNPs', 'lm_enriched_SNPs',
                                                             'lm_all_SNPs', 'lm_all_SNPs_env')
)

all_traits_phenotypic_byEnvModel_aggregate = lapply(traits[-c(1, 2)],
                                                    create_prediction_comparison_plot_byModel,
                                                    blup_style = 'deregressed_blups_nostress_filterTrial',
                                                    columns = c('lm_PC5', 'lm_env', 'rf_env', 'rf_env_PC5')
)

(all_traits_prediction_plots_aggregate = plot_grid(
  plot_grid(plotlist = lapply(all_traits_phenotypic_byModel_aggregate, function(p) {p + theme(legend.position = 'none', text = element_text(size = 6))}), 
            nrow = 1, ncol = 5, labels = c('', 'A', 'B', 'C', 'D'),
            legend = get_legend(all_traits_prediction_plot_list_aggregate[[1]] + theme_bw() +
                                  theme(legend.position = 'left',text = element_text(size = 6)))),
  plot_grid(plotlist = lapply(all_traits_phenotypic_byEnvModel_aggregate, function(p) {p + theme(legend.position = 'none', text = element_text(size = 6))}), 
            nrow = 1, ncol = 5, labels = c('', 'E', 'F', 'G', 'H'),
            legend = get_legend(all_traits_prediction_plot_list_environment_aggregate[[1]] + theme_bw() +
                                  theme(legend.position = 'left', text = element_text(size = 6)))),
  nrow = 2
))

ggsave('all_traits_prediction_plots_aggregate.tif',
...
all_traits_phenotypic_byTrial_aggregate = lapply(traits[-c(1, 2)],
                                                 create_prediction_comparison_plot_byTrial,
                                                 blup_style = 'deregressed_blups_nostress_filterTrial',
                                                 columns = c('lm_PC5', 'lm_matching_SNPs', 'lm_enriched_SNPs',
                                                             'lm_all_SNPs', 'lm_all_SNPs_env')
)

all_traits_phenotypic_byTrial_environmental_aggregate = lapply(traits[-c(1, 2)],
                                                               create_prediction_comparison_plot_byTrial,
                                                               blup_style = 'deregressed_blups_nostress_filterTrial',
                                                               columns = c('lm_PC5', 'lm_env', 'rf_env', 'rf_env_PC5')
)
...
(all_traits_prediction_plots = 
    plot_grid(plotlist = lapply(all_traits_phenotypic_byTrait_aggregate, function(p) {p + theme(legend.position = 'none', text = element_text(size = 8))}), 
              nrow = 3, ncol = 2, labels = c('', 'A', 'B', 'C', 'D'),
              legend = get_legend(all_traits_phenotypic_byTrait_aggregate[[1]] + theme(legend.position = 'left', text = element_text(size = 8))))
)
...
(all_traits_prediction_plots_byEnv = 
    plot_grid(plotlist = lapply(all_traits_phenotypic_byTrial_environmental_aggregate, function(p) {p + theme(legend.position = 'none', text = element_text(size = 8))}), 
              nrow = 3, ncol = 2, labels = c('', 'A', 'B', 'C', 'D'),
              legend = get_legend(all_traits_phenotypic_byTrial_environmental_aggregate[[1]] + theme(legend.position = 'left', text = element_text(size = 8))))
)
...
##### direct comparison of enriched SNPs against matching SNPs or just PCs (unused)  ##############################################
...
##### phenotypic prediction for unstructured envGWAS w/o pop struct correction  ##############################################


all_traits_prediction_plot_list_unstructured = lapply(traits[-c(1, 2)], 
                                                      create_prediction_comparison_plot_byModel,
                                                      blup_style = 'unstructured_deregressed_blups_nostress_filterTrial',
                                                      columns = c('lm_PC5', 'lm_all_SNPs',
                                                                  'lm_unstructured_SNPs', 'lm_unstructured_matching_SNPs')
)
...
ggsave('unstructured_envGWAS.tif',
       plot = unstructured_results,
       device = 'tiff',
       here(plot_dir, 'Manuscript'),
       width = 6,
       height = 7,
...
unstructured_phenotypic_byTrial_aggregate = lapply(traits[-c(1, 2)],
                                                 create_prediction_comparison_plot_byTrial,
                                                 blup_style = 'unstructured_deregressed_blups_nostress_filterTrial',
                                                 columns = c('lm_PC5', 'lm_all_SNPs',
                                                             'lm_unstructured_SNPs', 'lm_unstructured_matching_SNPs')
...
unstructured_phenotypic_byTrial_aggregate[[4]]
...
##### deprecated plots for various talks, etc ###############################################


## plot combining environmental map and transfer plots - 
...
# phenotypic prediciton for BAPG talk
# plot_BAPG = lapply(traits[-c(1, 2)], 
#                    create_prediction_comparison_plot_byModel,
#                    blup_style = 'deregressed_blups_nostress_filterTrial',
#                    columns = c('lm_PC5', 'lm_enriched_SNPs', 'lm_all_SNPs',
...
# bar plot for all models FOR ONE TRAIT - JUST FOR TESTING
...
# 
# ## plot improvement of environment model against just PCs - JUST FOR TESTING
