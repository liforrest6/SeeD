...
####################################################################################
## generate brms models for transfer plots to model - do not use if generating models
## from Optimal_model_environment_unconstrained.R
####################################################################################
...
ggsave('transfer_plots_brms.pdf',
       ...
       device = 'pdf',
       here(plot_dir, 'unconstrained'), ## change for unconstrained
       width = 160,
       height = 80,
       units = 'mm'
)
...
ggsave('slope_posterior_plot.pdf',
       ...
       device = 'pdf',
       here(plot_dir, 'unconstrained'), ## change pdf location
       width = 3,
       height = 3,
       units = 'in'
)
...
test_file = files[12]
getTrialPosteriors_independent(test_file)
...
# ## generates a posterior for one trial for use in talk
