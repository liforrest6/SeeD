...
pdf(file.path(dir, 'Analyses/Spatial/spatial_cluster_map.pdf'))
autoplot(clusters)
dev.off()
...
combined_results = foreach(i_fold = 1:nrow(clusters),.combine=rbind) %do%   {
  ...
  training = analysis(clusters$splits[[i_fold]])
  testing = assessment(clusters$splits[[i_fold]])

  fold_data = env_data

  testing_index = match(testing$V1,fold_data$Unique.ID)
